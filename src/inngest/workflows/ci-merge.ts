      ...
      if (startedAt + timeoutMs < Date.now()) {
        await step.run(`ci-timeout`, async () => {
          ...
          updateTaskFailure(
            db,
            linearIssueId,
            `CI timed out after ${config.deployTimeoutMin} minutes`,
            "ci",
          );
            ...
            async (): Promise<{
              ...
              if (freshTask.reviewCycleCount < config.maxReviewCycles) {
                ...
                updateTaskFailure(
                  db,
                  linearIssueId,
                  `CI failed and review cycles exhausted (${config.maxReviewCycles}/${config.maxReviewCycles})`,
                  "ci",
                );
                emitTaskUpdated(getTask(db, linearIssueId)!);
    ...
    if (mergeState.mergeStateStatus === "BEHIND") {
      ...
      if (task.reviewCycleCount < config.maxReviewCycles) {
        ...
        updateTaskFailure(
          db,
          taskId,
          `Merge conflicts with review cycle limit reached (${config.maxReviewCycles}/${config.maxReviewCycles})`,
          "ci",
        );
        emitTaskUpdated(getTask(db, taskId)!);
          ...
          if (rebaseResult.hasConflicts) {
            ...
            if (task.reviewCycleCount < config.maxReviewCycles) {
              ...
              updateTaskFailure(
                db,
                taskId,
                `Rebase conflicts with review cycle limit reached (${config.maxReviewCycles}/${config.maxReviewCycles})`,
                "ci",
              );
              emitTaskUpdated(getTask(db, taskId)!);
