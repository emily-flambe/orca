...
import { checkDrainState } from "../../scheduler/drain-monitor.js";
import { isDraining, getDrainStartedAt } from "../../deploy.js";
...
export const reconcileStuckTasksWorkflow = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    await step.run("check-drain-state", async () => {
      const deps = getSchedulerDeps();
      const draining = isDraining();
      const activeSessions = countActiveSessions(deps.db);
      const drainStartedAt = getDrainStartedAt();
      await checkDrainState(deps, draining, activeSessions, drainStartedAt);
    });
  },
);
