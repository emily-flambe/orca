...
export function createApiRoutes(deps: ApiDeps): Hono {
  ...
  app.post("/api/invocations/:id/prompt", async (c) => {
    ...
    const handle = activeHandles.get(id);
    if (!handle) {
      return c.json({ error: "no active process handle for this invocation" }, 409);
    }

    const sent = sendPrompt(handle, body.prompt.trim());
    if (!sent) {
      return c.json({ error: "process is not accepting input (may have already exited)" }, 409);
    }
