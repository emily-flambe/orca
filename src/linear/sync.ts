...
export function mapLinearStateToOrcaStatus(
  ...
  // Check overrides first (keyed by name)
...
function upsertTask(db: OrcaDb, issue: LinearIssue, config: OrcaConfig): void {
  ...
  const orcaStatus = mapLinearStateToOrcaStatus(issue.state.name, issue.state.type, config?.stateMapOverrides);
...
export async function processWebhookEvent(
  ...
  if (event.data.state) {
    ...
    resolveConflict(db, event.data.identifier, event.data.state.name, event.data.state.type, config);
...
export function resolveConflict(
  ...
  // Conflict case 8: deploying, Linear started+review → no-op (expected state, don't overwrite)
  if (task.orcaStatus === "deploying" && linearStateType === "started" && /review/i.test(linearStateName)) {
    return;
  }

  // Conflict case 8b: awaiting_ci, Linear started+review → no-op (expected state, don't overwrite)
  if (task.orcaStatus === "awaiting_ci" && linearStateType === "started" && /review/i.test(linearStateName)) {
    return;
  }
