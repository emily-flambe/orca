...
function mapLinearStateToOrcaStatus(
  ...
  // Check overrides first (keyed by Linear state name)
  if (overrides?.has(stateName)) {
    return overrides.get(stateName)!;
  }
  ...
  switch (stateType) {
    ...
    case "unstarted":
      return "ready";
    case "started":
      // Distinguish in_review from running by state name heuristic
...
function upsertTask(db: OrcaDb, issue: LinearIssue, config: OrcaConfig): void {
  ...
  const orcaStatus = mapLinearStateToOrcaStatus(issue.state.name, issue.state.type);
...
export function resolveConflict(
  ...
  if (linearStateType === "backlog") {
    ...
    log(
      `conflict resolved: task ${taskId} reset to backlog from ${task.orcaStatus} (Linear type: backlog)`,
    );
    return;
  }

  // Any state → Linear unstarted: reset to ready with fresh retry/review counts.
  ...
  if (linearStateType === "unstarted") {
    ...
    log(
      `conflict resolved: task ${taskId} reset to ready from ${task.orcaStatus} (Linear type: unstarted)`,
    );
    return;
  }
  ...
  if (task.orcaStatus === "in_review" && linearStateType === "completed") {
    updateTaskStatus(db, taskId, "done");
    log(
      `conflict resolved: task ${taskId} set to done from in_review (human override)`,
    );
    return;
  }
  ...
  if (task.orcaStatus === "deploying" && linearStateType === "completed") {
    updateTaskStatus(db, taskId, "done");
    log(
      `conflict resolved: task ${taskId} set to done from deploying (human override)`,
    );
    return;
  }
  ...
  if (task.orcaStatus === "awaiting_ci" && linearStateType === "completed") {
    updateTaskStatus(db, taskId, "done");
    log(
      `conflict resolved: task ${taskId} set to done from awaiting_ci (human override)`,
    );
    return;
  }
