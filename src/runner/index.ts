  ...
  handle.done = new Promise<SessionResult>((resolve) => {
    ...
    rl.on("line", (line: string) => {
      ...
      try {
        ...
        logger.warn(`non-JSON line from claude (invocation ${options.invocationId}): ${line.slice(0, 200)}`);
        return;
      }
    ...
    if (proc.stderr) {
      proc.stderr.on("data", (chunk: Buffer) => {
        logger.warn(`[stderr][inv-${options.invocationId}] ${chunk.toString().trimEnd()}`);
      });
    }
