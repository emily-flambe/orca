...
export interface SessionResult {
  ...
  costUsd: number | null;
  /** Total input tokens (includes cache creation + cache read), if reported by the CLI. */
  inputTokens: number | null;
  /** Total output tokens, if reported by the CLI. */
  ...
  handle.done = new Promise<SessionResult>((resolve) => {
    ...
    rl.on("line", (line: string) => {
      ...
      if (type === "result") {
        ...
        // Parse token usage from the `usage` field.
        const usage =
        ...
        if (usage) {
          ...
          inputTokens = raw > 0 ? raw : null;
          const outRaw = Number(usage.output_tokens ?? 0);
          outputTokens = outRaw > 0 ? outRaw : null;
        }
