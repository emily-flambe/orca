...
import { loadConfig, parseRepoPath, validateProjectRepoPaths } from '../config/index.js';
...
program
  .name('orca')
  .description('AI agent scheduler — dispatches and manages Claude Code sessions');
...
program
  ...
  .action((opts: { prompt: string; repo: string; priority: string; id?: string }) => {
...
program
  ...
  .action(async () => {
    ...
    const projectMeta = await client.fetchProjectMetadata(config.linearProjectIds);
    ...
    if (orphanedInvocations.length > 0) {
      console.log(`[orca] marked ${orphanedInvocations.length} orphaned invocation(s) as failed`);
    }

    // Now recover orphaned tasks: any task stuck in "running" or
    // "dispatched" with no running invocation is dead.
    const allTasks = getAllTasks(db);
    const runningInvIssueIds = new Set(getRunningInvocations(db).map((inv) => inv.linearIssueId));
    let recovered = 0;
...
program
  ...
  .action(() => {
    ...
    const failedCount = allTasks.filter((t) => t.orcaStatus === 'failed').length;
