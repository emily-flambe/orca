                ...
                <Fragment key={inv.id}>
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <span className="text-xs text-gray-400 tabular-nums">{formatDuration(inv.startedAt, inv.endedAt)}</span>
                      {inv.costUsd != null && (
                        <span className="text-xs text-gray-400 tabular-nums">${inv.costUsd.toFixed(2)}</span>
                      )}
                      {inv.numTurns != null && (
                      ...
                      {inv.status === "running" && (
                        <button
                          ...
                          className="ml-auto text-xs px-2 py-0.5 rounded bg-red-500/20 text-red-400 active:bg-red-500/30 transition-colors"
                        >
                          Abort
                        </button>
                      )}
                    </div>
                    <div className="text-xs text-gray-500">{formatDate(inv.startedAt)}</div>
                    {inv.outputSummary && (
                      <div className="mt-1.5 text-xs text-gray-400 line-clamp-2">{inv.outputSummary}</div>
                    )}
                  </div>
