                    ...
                    <div className="flex items-center justify-between gap-2 mb-1.5">
                      ...
                      {inv.status === "running" && (
                        <button
                          ...
                          className="text-xs px-2 py-0.5 rounded bg-red-500/20 text-red-400 active:bg-red-500/30 transition-colors"
                        >
                          Abort
                        </button>
                      )}
                    </div>
                    <div className="text-xs text-gray-400 mb-1">{formatDate(inv.startedAt)}</div>
                    <div className="flex gap-3 text-xs text-gray-300 tabular-nums mb-1">
                      <span>{formatDuration(inv.startedAt, inv.endedAt)}</span>
                      <span>{inv.costUsd != null ? `$${inv.costUsd.toFixed(2)}` : "—"}</span>
                      {inv.numTurns != null && <span>{inv.numTurns} turns</span>}
                    </div>
                    {inv.outputSummary && (
                      <p className="text-xs text-gray-400 line-clamp-2">{inv.outputSummary}</p>
                    )}
                  </div>
