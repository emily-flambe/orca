...
// ---------------------------------------------------------------------------
// POST /api/invocations/:id/prompt  (EMI-84: send prompt to running session)
// ---------------------------------------------------------------------------
...
describe("POST /api/invocations/:id/prompt", () => {
  ...
  beforeEach(async () => {
    ...
    // Mock sendPrompt and activeHandles at module level
    sendPromptMock = vi.fn().mockReturnValue(true);
    vi.doMock("../src/runner/index.js", () => ({
      sendPrompt: sendPromptMock,
      killSession: vi.fn().mockResolvedValue(undefined),
      invocationLogs: vi.fn().mockReturnValue(null),
    }));
    vi.doMock("../src/scheduler/index.js", () => ({
  ...
  function postPrompt(invocationId: number | string, body: unknown) {
  ...
  it("returns 400 for non-numeric invocation id", async () => {
    const res = await postPrompt("abc", { prompt: "hello" });
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toMatch(/invalid invocation id/i);
  });

  it("returns 404 when invocation not found in db", async () => {
    const res = await postPrompt(9999, { prompt: "hello" });
    expect(res.status).toBe(404);
    const body = await res.json();
    expect(body.error).toMatch(/not found/i);
  });

  it("returns 409 when invocation is not running (completed)", async () => {
    ...
    // Get the inserted invocation id
    const { getInvocationsByTask } = await import("../src/db/queries.js");
    const invocations = getInvocationsByTask(db, "PROMPT-TASK");
    const id = invocations[0].id;

    const res = await postPrompt(id, { prompt: "hello" });
    expect(res.status).toBe(409);
    const body = await res.json();
    expect(body.error).toMatch(/not running/i);
  });

  it("returns 409 when invocation is not running (failed)", async () => {
    insertTask(db, makeTask({ linearIssueId: "PROMPT-TASK-2" }));
    insertInvocation(db, {
      linearIssueId: "PROMPT-TASK-2",
      startedAt: now(),
      status: "failed",
    });
    const { getInvocationsByTask } = await import("../src/db/queries.js");
    const invocations = getInvocationsByTask(db, "PROMPT-TASK-2");
    const id = invocations[0].id;
  ...
  it("returns 400 when prompt is missing", async () => {
    insertTask(db, makeTask({ linearIssueId: "PROMPT-TASK-3" }));
    insertInvocation(db, {
      linearIssueId: "PROMPT-TASK-3",
      startedAt: now(),
      status: "running",
    });
    const { getInvocationsByTask } = await import("../src/db/queries.js");
    const invocations = getInvocationsByTask(db, "PROMPT-TASK-3");
    const id = invocations[0].id;

    const res = await postPrompt(id, {});
    expect(res.status).toBe(400);
    const body = await res.json();
  ...
  it("returns 400 when prompt is empty string", async () => {
    insertTask(db, makeTask({ linearIssueId: "PROMPT-TASK-4" }));
    insertInvocation(db, {
      linearIssueId: "PROMPT-TASK-4",
      startedAt: now(),
      status: "running",
    });
    const { getInvocationsByTask } = await import("../src/db/queries.js");
    const invocations = getInvocationsByTask(db, "PROMPT-TASK-4");
    const id = invocations[0].id;

    const res = await postPrompt(id, { prompt: "   " });
    expect(res.status).toBe(400);
  });

  it("returns 400 when prompt is not a string", async () => {
    insertTask(db, makeTask({ linearIssueId: "PROMPT-TASK-5" }));
    insertInvocation(db, {
      linearIssueId: "PROMPT-TASK-5",
      startedAt: now(),
      status: "running",
    });
    const { getInvocationsByTask } = await import("../src/db/queries.js");
    const invocations = getInvocationsByTask(db, "PROMPT-TASK-5");
    const id = invocations[0].id;

    const res = await postPrompt(id, { prompt: 42 });
    expect(res.status).toBe(400);
  });

  it("returns 400 for invalid JSON body", async () => {
    insertTask(db, makeTask({ linearIssueId: "PROMPT-TASK-6" }));
    insertInvocation(db, {
      linearIssueId: "PROMPT-TASK-6",
      startedAt: now(),
      status: "running",
    });
    const { getInvocationsByTask } = await import("../src/db/queries.js");
    const invocations = getInvocationsByTask(db, "PROMPT-TASK-6");
    const id = invocations[0].id;

    const res = await app.request(`/api/invocations/${id}/prompt`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: "not-json{{{",
    });
  ...
  it("returns 404 when invocation is running but no active handle", async () => {
    // activeHandles is empty (mocked above), so no handle for the invocation
    insertTask(db, makeTask({ linearIssueId: "PROMPT-TASK-7" }));
    insertInvocation(db, {
      linearIssueId: "PROMPT-TASK-7",
      startedAt: now(),
      status: "running",
    });
    const { getInvocationsByTask } = await import("../src/db/queries.js");
    const invocations = getInvocationsByTask(db, "PROMPT-TASK-7");
    const id = invocations[0].id;
