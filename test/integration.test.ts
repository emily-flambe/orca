...
describe("9.5 - Budget enforcement", () => {
  ...
  test("events outside the window are excluded", () => {
    ...
    // Only the recent 500 tokens should be counted, not the old 10,000,000
  ...
  test("budget check logic: dispatch blocked when tokens >= max", () => {
    const budgetMaxTokens = 10_000;
    const budgetWindowHours = 4;

    const taskId = seedTask(db, { linearIssueId: "BUDGET-BLOCK" });
    const invId = seedInvocationForBudget(db, taskId);

    // Insert events totaling 10,500 tokens (over budget)
    insertBudgetEvent(db, {
      invocationId: invId,
      costUsd: 0,
      inputTokens: 7_000,
    ...
    // Budget is exhausted: tokens (10,500) >= max (10,000)
    expect(tokensUsed).toBeGreaterThanOrEqual(budgetMaxTokens);
    ...
    const shouldSkipDispatch = tokensUsed >= budgetMaxTokens;
  ...
  test("budget check logic: dispatch allowed when tokens < max", () => {
    ...
    expect(tokensUsed).toBeLessThan(budgetMaxTokens);
